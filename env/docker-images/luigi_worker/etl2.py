...
class Plot(ExternalProgramTask):
    ...
    def requires(self):
        return SentimentAnalysis(listing = self.listing, idReg=-1)
    
class SentimentAnalysis(ExternalProgramTask):
    listing = luigi.DateMinuteParameter(default=datetime.date.today())
    idReg = luigi.IntParameter(default=0)

    def output(self):
        return luigi.LocalTarget("/datalake/clas/{}.clas".format(self.requires().idReg))

    def program_args(self):
        self.idReg=self.requires().idReg
        self.prediction=0.10
        args = ["bash", "sentiment.sh", self.requires().idReg]
        return args

    def requires(self):
        return GetSentences(listing = self.listing, idReg=-1)

class GetSentences(ExternalProgramTask):
    listing = luigi.DateMinuteParameter(default=datetime.date.today())
    idReg = luigi.IntParameter(default=0)

    def output(self):
        return luigi.LocalTarget("/datalake/sen/{}.sen".format(self.requires().idReg))

    def program_args(self):
        self.idReg=self.requires().idReg
        self.prediction=0.10
        args = ["bash", "getSentences.sh", self.requires().idReg]
        return args

    def requires(self):
        return TranscribeAudio(listing = self.listing, idReg=-1)

class TranscribeAudio(ExternalProgramTask):
    listing = luigi.DateMinuteParameter(default=datetime.date.today())
    idReg = luigi.IntParameter(default=0)

    def output(self):
        return luigi.LocalTarget("/datalake/trans/{}.tra".format(self.requires().idReg))

    def program_args(self):
        self.idReg=self.requires().idReg
        self.prediction=0.10
        args = ["bash", "transcribe.sh", self.requires().idReg]
        return args
